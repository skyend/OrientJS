var _ = function (_editor) {
    ...
    this.placeholder = function (_targetStack) {
        ...
        var triangleLength = 8, triangleHeight = Math.sqrt((Math.pow(triangleLength, 2) - Math.pow(triangleLength / 2, 2)));
        var linePointX = triangleLength / 2, lineLength = targetNodeoffset.height;
        var width = triangleLength + 2, height = lineLength + (triangleHeight * 2);
        if(!horizontality){
            var _temp = width;
            width = height;
            height = _temp;
