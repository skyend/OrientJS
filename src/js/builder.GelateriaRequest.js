...
class GelateriaRequest {
  ...
  createAPIInterface(_serviceId, _title, _complete) {
    console.log('create');
    request.post("http://localhost:8080/" + ["apiinterface", 'create'].join("/"))
      .type('form')
      .withCredentials()
      .send({
        serviceId: _serviceId,
  ...
  getAPIInterfaceList(_serviceId, _complete) {
    request.post('http://localhost:8080/' + ['apiinterface', 'list'].join('/'))
      .type('form')
      .withCredentials()
      .send({
        serviceId: _serviceId,
  ...
  loadApiinterface(_service_real_id, _apiinterfaceId, _complete) {
    console.log("Service REAL ID", _service_real_id);
    request.post("http://localhost:8080/apiinterface/" + ["retrieve"].join("/"))
      .type('form')
      .withCredentials()
      .send({
        'service_real_id': _service_real_id,
