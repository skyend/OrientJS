...
var Request = React.createClass({
    ...
    testRequest () {
      ...
      this
        .props
        .contextController
        .executeRequestTest(this.props.request, this.getFields(), this.getHeaders(), function(_result) {
          console.log(_result);

          self.setState({
            icafeResult: _result,
            showDataPreviewer: true,
            loadingData: false
          });
        });
    },
    ...
    getFields () {
      ...
      if (this.props.request.fieldFillFromNodeType) {
        if (this.props.nodeTypeData === null) 
          throw new Error("노드타입데이터가 로드되지 않았습니다.");
        
        let self = this;

        let propertyTypes = Object
          .keys(this.props.nodeTypeData.propertytype)
          .map(function(_key) {
    ...
    changeURLPattern () {
      ...
      this.props.request.customUrlPattern = this.refs['url-pattern']
        .getDOMNode()
        .value;
    ...
    changeCRUD () {
      ...
      var value = this.refs['crud-selector']
        .getDOMNode()
        .value;
    ...
    changeMethod () {
      ...
      var value = this.refs['method-selector']
        .getDOMNode()
        .value;
    ...
    renderFillFieldsFromNodeType () {
      if (this.props.request.fieldFillFromNodeType !== true) 
        return '';
      if (this.props.nodeTypeData === null) 
        return '';
      ...
      var testFieldPlaceHolders = this.props
        .contextController
        .getRequestTestFieldPlaceholder(this.props.request.name);
      ...
      return propertyTypes.map(function(_pt, _i) {
        ...
        return <div className='field'>
          ...
          <input placeholder="Test value" value={testFieldPlaceHolders[_pt.pid]} onChange={function(_e) {
            self.changeFieldTestValue(_pt.pid, _e);
          }}/>
        </div>
      })
    },
    ...
    renderURLPatternInput () {
      ...
      if (this.props.request.crud !== "*") {
        let urlPattern = contextController.serviceManager.iceHost + "/api/" + (contextController.nodeTypeId !== undefined
          ? contextController.nodeTypeId
          : "{tid}") + "/" + this.props.request.crud + ".json";
    ...
    renderRows () {
      ...
      if (!this.props.isInterface) 
        testFieldPlaceHolders = this.props
          .contextController
          .getRequestTestFieldPlaceholder(this.props.request.name);
      
      return [ < div className = 'row' > <label>
        URL Pattern
      </label> < div className = 'request-pattern' > {
    ...
    <div className='row'>
      <label> Method </label > <select onChange={this.changeMethod} value={this.props.request.method} ref='method-selector'>
    ...
    <div className='row'>
      <label> Headers </label > <div className='request-field-set'>
        <div className='configs'>
          <div className='config'>
            {this.renderWithCheckEditable(
              <button onClick={this.addNewHeader}>
                Header
        ...
        {this
          ...
          .map(function(_header, _i) {
            ...
            return <div className='field'>
              <input placeholder="Header name (a-z,_,-)" value={_header.name} onChange={function(_e) {
                self.changeHeaderName(_i, _e);
              }}/>
              <i className='fa fa-long-arrow-right'/>
              <input placeholder="Header value or resolver" value={_header.value} onChange={function(_e) {
                self.changeHeaderValue(_i, _e);
              }}/>
              <i className='fa fa-ellipsis-v'/>
              <input placeholder="Test value" value={_header.testValue} onChange={function(_e) {
                self.changeHeaderTestValue(_i, _e);
              }}/>
              {self.renderWithCheckEditable(
                <button onClick={function() {
                  self.deleteHeader(_i)
                }}>Delete</button>
              )}
            </div>
          })}
    ...
    <div className='row'>
      <label> Fields </label > <div className='request-field-set'>
        <div className='configs'>
          <div className='config'>
            <div className='title'>
        ...
        {this
          ...
          .map(function(_field, _i) {
            return <div className='field'>
              <input placeholder="Field name (a-z,_,-)" value={_field.name} onChange={function(_e) {
                self.changeFieldName(_i, _e);
              }}/>
              <i className='fa fa-long-arrow-right'/>
              <input placeholder="Field value or resolver" value={_field.value} onChange={function(_e) {
                self.changeFieldValue(_i, _e);
              }}/>

              {self.props.isInterface
                ? ''
                : [
                  < i className = 'fa fa-ellipsis-v' />, < input placeholder = "Test value" value = {
                    testFieldPlaceHolders[_field.name]
                  }
                  onChange = {
                    function(_e) {
                      self.changeFieldTestValue(_field.name, _e);
                    }
                  } />
                ]}
              {self.renderWithCheckEditable(
  ...
  render () {
    ...
    return <div className={'request ' + (this.isFromInterface
        ? "smaller from-interface"
        : '')}>
        <div className='row'>
          <button onClick={this.toggleFold} className=''>
            {this.state.fold
          ...
          {this.isFromInterface
            ? <div className='interface-name'>
                <i className='fa fa-plug'/>
                {this.props.interface.title}
              </div>
            : ''}
          ...
          <button onClick={this.testRequest}>
            <i className={'fa fa-refresh ' + (this.state.loadingData
              ? "fa-spin"
              : '')}/>
            Test
          </button>
          <button onClick={this.testRequestWithChain}>
            <i className={'fa fa-refresh ' + (this.state.loadingDataWithChain
              ? "fa-spin"
              : '')}/>
            Test With Chain</button>
...
.createClass({
  ...
  toggleInterfaceAdder () {
    this.setState({
      showInterfaceAdder: !this.state.showInterfaceAdder
    });
  },
  ...
  createRequest (_e) {
    let name = this.refs['request-name']
      .getDOMNode()
      .value;
    let crud = this.refs['crud-selector']
      .getDOMNode()
      .value;
    ...
    this.props
      .contextController
      .addRequest(name, crud);

    this.forceUpdate();
  },

  onThrowCatcherUpdatedRequest (_eventData) {
    if (_eventData.isImplemented) 
      return;
    
    // implemented 의 request 는 저장할 수 없다.

    this.props
      .contextController
      .updateRequest(_eventData.request);
  ...
  onThrowCatcherUpdatedRequestPlaceholder (_eventData) {
    ...
    switch (object.target) {
      case "testField":
        this.props
          .contextController
          .setRequestTestFieldPlaceholder(requestName, object.name, object.value);
        break;
      default:
  ...
  onThrowCatcherDeleteRequest (_eventData) {
    this.props
      .contextController
      .deleteRequest(_eventData.request);
  ...
  componentDidUpdate () {
    if (this.props.runningState === this.props.contextController.running) 
      return;
  ...
  componentDidMount () {
    ...
    if (this.props.contextType === 'apiSource') {
      this
        .contextController
        .getNodetypeData(function(_result) {
          self.setState({
  ...
  renderCRUDList () {
    if (this.props.contextType === 'apiSource') {
      return [
        ...
        <option value="READ">Read</option >, < option value = "*" > CRUD Free < /option>]
  ...
  renderCRUDPicker(){
    return <select ref='crud-selector'>
      ...
      renderRequestEditor () {
        ...
        return [ < div className = 'new-form' > <span className='field-name'>
          Request Name
        </span> < input ref = 'request-name' /> <span className='field-name'>
          Main CRUD
        </span>
          {
            ...
            this.createRequest
          } > <i className='fa fa-plus'/>Add Request < /button>
      </div >, < div className = 'request-list' > {
            this
              .state
              .followInterfaces
              .map(function(_interface) {
                return Object
                  .keys(_interface.requests || {})
                  .map(function(_requestKey) {
  ...
  renderInterfaceAdder(){
    ...
    <button onClick={function(_e){ _e.stopPropagation(); self.addInterface(self.refs['interface-selector'].getDOMNode().value); }}>
      <i className='fa fa-plus'/ > </button>, < button > <i className='fa fa-times'/> < /button>];
  },
  ...
  renderNodeTypeDetail(){
    ...
    return (
      <table className='nodetype-list'>
        ...
        <tr className='item'>
          <td className='item-title' width='150px'>
            <span className='title'>Parent Nodetype</span > </td> < td className = 'item-description' > <span className='block-merge'>
        {function() {
          var items = [];
          if (this.state.nodeTypeData.tree.display !== undefined) {
            items.push(
              <span className='block display-field'>{this.state.nodeTypeData.tree.display}</span>
            );
          }
          if (this.state.nodeTypeData.tree.value !== undefined) {
            items.push(
              <span className='block value-field'>{this.state.nodeTypeData.tree.value}</span>
            );
          }
          if (this.state.nodeTypeData.tree.nid !== undefined) {
            items.push(
              <span className='block nid-field'>{this.state.nodeTypeData.tree.nid}</span>
            );
          }
          return items;
        }.apply(this)}
        ...
        <td className='item-description'>
          {Object
            .keys(this.state.nodeTypeData.propertytype)
            .map(function(_key) {
      ...
      </tr> < tr className = 'item' > <td className='item-title' width='150px'>
        <span className='title'>CRUD</span>
      </td> < td className = 'item-description' > {
        this
          .state
          .nodeTypeData
          .crud
        ...
        <td className='item-description'>
          ...
          {this
            .props
            .contextController
            .followedInterfaceList
            .map(function(_apiInterfaceId) {
          ...
          <span className={'block display-field plus-button ' + (this.state.showInterfaceAdder
            ? 'select-box-width-plus'
            : '')} onClick={this.toggleInterfaceAdder}>
            {this.state.showInterfaceAdder
              ? this.renderInterfaceAdder()
              : <i className='fa fa-plus'/>}
          </span>
        </td>
      </tr> < /table>
  ...
  renderNodeTypeSpecInfo(){
    ...
    }
    {
      ...
      this.props.contextController.nodeTypeId
        ...
        </small > </h1> < div className = 'detail' > {
      this.state.nodeTypeData === null
        ? <i className="fa fa-spinner fa-pulse loading"/>
        : this.renderNodeTypeDetail()
    } < /div>
    </div >;
}, renderInterfaceInfo () {
  ...
  let nodeRenderZoneStyle = {
    display: 'none'
  };
  return (

    <div className='APISourceContext dark' style={style}>
      {this.props.contextType === 'apiSource'
        ? this.renderNodeTypeSpecInfo()
        : this.renderInterfaceInfo()}
      ...
      <div className='request-navigation'></div>

      <div className='node-render-zone' style={nodeRenderZoneStyle}>
        {this.state.nodeTypeData !== null
          ? 'this.renderDataZone()'
          : ''}
      </div>
