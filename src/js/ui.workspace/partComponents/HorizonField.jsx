...
var EnterableWrapperCodeEditor = React.createClass({
  ...
  render () {
    ...
    return (
      <div className='ace'>
        <AceEditor mode={this.props.lang} theme="twilight" onChange={this.onChange} name={this.props.editorId} value={this.state.value} width='100%' height='100%' editorProps={editorProps}/>
      </div>
    );
  }
});
...
var HorizonField = React.createClass({
  ...
  nameDoubleClick (_e) {
    if (!this.props.editorableFieldName) 
      return;
    
    if (this.state.fieldNameEditMode) {
      var input = this.refs['FieldNameInput']
        .getDOMNode();
    ...
    this.setState({
      fieldNameEditMode: !this.state.fieldNameEditMode
    });
  ...
  onGlobalDragFromUI (_e) {
    ...
    if (this.prevX !== undefined) {
      ...
      if (this.refs['field-option-box'] === undefined) 
        return;
      var optionBoxDom = this.refs['field-option-box']
        .getDOMNode();
  ...
  getValue () {
    if (this.props.enterable) {
      return this.refs['enterable-field']
        .getValue();
    } else {
  ...
  render () {
    var classes = [
      'HorizonField', this.props.theme, this.props.type
    ];
    var field;
    var iconClass;
    ...
    let fieldValueStyle = {
      left: this.props.nameWidth
    };
    let fieldNameBoxStyle = {
      left: this.state.nameBoxSlideX
    };
    let fieldNameStyle = {
    ...
    return (
      <div className={classes.join(' ')} style={style}>
        <div className="field-name" style={fieldNameStyle} onMouseDown={this.fieldNameMouseDown}>
          ...
          <div className='field-name-box' ref='field-name-box' style={fieldNameBoxStyle} onDoubleClick={this.nameDoubleClick}>
            <div className='vertical-standard'></div>
            {fieldNameRender}
          </div>
        ...
        <div className={[
          'field-value', this.state.valueIsDiff
            ? 'diff'
            : ''
        ].join(' ')} style={fieldValueStyle}>
          <div className='vertical-standard'></div>
          {field}
        </div>
