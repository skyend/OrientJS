...
(function() {
  ...
  var ReactClass = React.createClass({
    ...
    getItemElement (_item, _index) {
      ...
      return (
        <li className={classes.join(' ')} value={_item} onClick={this.selectItem} data-index={_index}>{_item}</li>
      )
    },
    ...
    cycleItemCursor (_direct) {
      ...
      if (_direct === 'down') {
        ...
        if (nextCursor >= this.props.itemCount) 
          return -1;
        }
      else {
        nextCursor = currentCursor - 1;
    ...
    selectItem (_e) {
      ...
      this.setState({
        itemCursor: parseInt(_e.target.getAttribute('data-index'))
      });
    },
    ...
    changedValue () {
      ...
      if (typeof this.props.onChanged === 'function') {
        this.props
          .onChanged(this.props.fieldName, value);
      }
    },
    ...
    render () {
      var classes = [
        'InputBoxWithSelector', this.props.color
      ];
      if (typeof this.props.selectorItems === 'object') {
      ...
      let style = {
        'display': this.state.spreadDisplay
      };

      return (
        <div className={classes.join(' ')}>
          <div className="input-wrapper">
            <input autoComplete="off" className={this.props.dontEnter
              ? 'clickable'
              : ''} onClick={this.inputClick} onKeyUp={this.onKeyUp} onChange={this.changedValue} ref='input' disabled={this.props.dontEnter}/>
            <div className='spread-out' onClick={this.toggleSpread}>
              <i className='fa fa-sort-down'></i>
            </div>

            <div className="selector-wrapper" style={style} ref='spread'>
              <ul className='selector'>
                {this
                  .props
                  .selectorItems
                  .map(this.getItemElement)}
              </ul>
            </div>
          </div>
