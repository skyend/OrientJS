...
var IFrameStage = React.createClass({
  ...
  getIframeInnerWindow () {
    var iframeDom = this.refs['iframe']
      .getDOMNode();
  ...
  addStyle (_key, _style) {
    ...
    if (typeof this.componentStyleElement === 'undefined') {
      var styleTag = this.getIFrameInnerDoc()
        .createElement('style');
  ...
  getElementByVid (_vid) {
    return this.getIFrameInnerDoc()
      .querySelector('[__vid__="' + _vid + '"]');
  },

  appendStyleElement (_element) {
    this.getIFrameInnerDoc()
      .head
  ...
  insertElementToInLastBySelector (_selector, _element) {
    var baseTarget = this.getIFrameInnerDoc()
    ...
    if (childNodes.length > 0) {
      ...
      for (var i = 0; i < childNodes.length; i++) {
        ...
        for (var j = 0; j < rects.length; j++) {
          ...
          if ((rects[j].left < _mx && rects[j].right > _mx) && (rects[j].top < _my && rects[j].bottom > _my)) {
