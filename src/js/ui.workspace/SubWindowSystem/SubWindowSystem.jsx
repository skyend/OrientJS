(function() {
  ...
  var SubWindowSystem = React.createClass({
    ...
    onThrowCatcherFocusedMe (_eventData, _pass) {
      ...
      if (focusedWindowRef < 0) 
        throw new Error("존재하지 않는 윈도우 Key입니다.");
    ...
    onThrowCatcherCloseMe (_eventData, _pass) {
      ...
      this.setState({
        subWindowItems: filteredSubWindowItems
      });
    },
    ...
    spawnSubWindow (_subWindowKey, _allowDuplicate, _newSubWindowItem) {
      ...
      if (duplicated) {
        ...
        if (!_allowDuplicate) 
          return this.refs[_newSubWindowItem.ref]
            .focus();
    ...
    renderWindowItem (_subWindowItem) {
      ...
      if (_subWindowItem.empty) {
        ...
        return (
          <SubWindow ref={_subWindowItem.ref} title={_subWindowItem.title} descType={_subWindowItem.descType} toolEgg={_subWindowItem.toolEgg} text={_subWindowItem.text} x={50} y={50} width={300} height={200} baseZOrder={this.state.startZIndex} zOrder={_subWindowItem.zOrder}/>
        );
      }
    },
