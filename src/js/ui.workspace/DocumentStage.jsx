...
(function() {
  ...
  var DocumentStage = React.createClass({
    ...
    openContext (_contextSpec) {
      var alreadyContextIndex = _.findIndex(this.state.contexts, {
      ...
      if (alreadyContextIndex != -1) {
        ...
        // 컨텍스트를 앞으로 가져온다.
        this.setState({
          runningContextID: _contextSpec.contextID
        });
      } else {
    ...
    clickTabItem (_contextItem) {
      this.setState({
        runningContextID: _contextItem.contextID
      });
    ...
    startDeployComponentByPalette (_absoluteX, _absoluteY, _componentKey, _packageKey) {
      if (!this.hasCurrentRunningContext()) 
        return;
      var context = this.getCurrentRunningContext();
      if (!context.isAcceptDropComponent()) 
        return;
      ...
      this.liveVDomController
        .createVRoot(iframeStageInnerDoc.querySelectorAll('html').item(0));
    ...
    dragDeployComponentByPalette (_absoluteX, _absoluteY, _componentKey, _packageKey) {
      ...
      if (this.guideBoxLive) {
        ...
        if (Math.abs(_absoluteX - this.prevMouseX) > this.props.aimingEscapeStepSize || Math.abs(_absoluteY - this.prevMouseY) > this.props.aimingEscapeStepSize) {
          // 표적해제
          this.clearAim();
        } else {
          // 움직임이 비교정 미세하다면
          ...
          this.emit("GetComponent", {
            ...
            "return": function(_err, _component) {
              if (_err !== null) 
                throw new Error("fail to component get");
      ...
      if (!((_absoluteX > selfClientRect.left && _absoluteX < selfClientRect.left + selfClientRect.width) && (_absoluteY > selfClientRect.top && _absoluteY < selfClientRect.top + selfClientRect.height))) {
    ...
    findGuideDirectBound (_absoluteX, _absoluteY) {
      ...
      var boundArray = [
        topBound, bottomBound, leftBound, rightBound, inBound
      ];
      var collision = null;
      for (var i = 0; i < boundArray.length; i++) {
        var bound = boundArray[i];
        var boundRect = bound.getDOMNode()
        ...
        // 충돌체크
        if ((boundRect.left < _absoluteX && boundRect.right > _absoluteX) && (boundRect.top < _absoluteY && boundRect.bottom > _absoluteY)) {
          collision = bound;
          //break;
        }
      }
      ...
      if (collision !== null) {
        // 지정 방향 표시
        collision.getDOMNode()
          .setAttribute('see', 'yes');
        return collision.getDOMNode()
    ...
    showPreviewComponentDeployPosition (_target, _direction, _positionHints) {
      ...
      if (direction === 'left' || direction === 'right') {
        ...
        x = direction === 'left'
          ? transformedCoordinate.x - boxWidth
          : transformedCoordinate.x + _target.element.offset.width;
        ...
        y = direction === 'top'
          ? transformedCoordinate.y - boxWidth
          : transformedCoordinate.y + _target.element.offset.height;
      ...
      placeHolderDOM.style.left = this.getCurrentRunningContext()
        .stageX + x + 'px';
      placeHolderDOM.style.top = this.getCurrentRunningContext()
        .stageY + y + 'px';
    ...
    changeDropPositionPlaceholderMode (_mode) {
      ...
      if (_mode !== undefined) {
        switch (_mode) {
          case "canInsert":
          case "cantInsert":
            this.getDropPositionPlaceholderDOMNode()
              .setAttribute('mode', _mode);
            break;
          default:
    ...
    getDropPositionPlaceholderDOMNode () {
      return this.refs['drop-position-placeholder']
        .getDOMNode();
    },
    ...
    transformIFrameDocCoordinateIntoStageScreen (_coordinate) {
      ...
      return {
        x: _coordinate.x - this.getCurrentRunningContext()
          .getIFrameStageScrollX() + this.getTabContextOffsetLeftByDS(),
        y: _coordinate.y - this.getCurrentRunningContext()
          .getIFrameStageScrollY() + this.getTabContextOffsetTopByDS()
      };
    },
    ...
    rayTracingListUp (_absoluteX, _absoluteY) {
      ...
      var selfClientRect = this.getCurrentRunningContext()
        .getIFrameStageBoundingRect();

      var checkX = _absoluteX - selfClientRect.left;
      var checkY = _absoluteY - selfClientRect.top + this.getCurrentRunningContext()
        .getIFrameStageScrollY();

      var collisions = this.liveVDomController
        .rayTracer(checkX, checkY);
    ...
    showGuideBox (_target, _absoluteX, _absoluteY) {
      ...
      var dropGuideBox = this.refs['drop-guide-box']
        .getDOMNode();
    ...
    showElementHighlight (_DOMElement, _mode) {
      ...
      highligher.style.left = this.getCurrentRunningContext()
        .stageX + boundingBox.left + 'px';
      highligher.style.top = this.getCurrentRunningContext()
        .stageY + boundingBox.top + this.getTabContextOffsetTopByDS() + 'px';
    ...
    getElementHighligherDOMElement () {
      return this.refs['element-highlighter']
        .getDOMNode();
    },
    ...
    changeElementHighlighterMode (_mode) {
      ...
      if (_mode !== undefined) {
        switch (_mode) {
          case "canDrop":
          case "cantDrop":
            this.getElementHighligherDOMElement()
              .setAttribute('mode', _mode);
            break;
          default:
            throw new Error("invailid mode " + _mode);
    ...
    componentDidMount () {
      if (this.getCurrentRunningContext()) {
        this.iframeStageBoundingRect = this.getCurrentRunningContext()
          .getIFrameStageBoundingRect();
      }
    ...
    renderContextTabItem (_contextItem) {
      ...
      return (
        <li className={running
          ? 'forwarded'
          : ''} onClick={closure}>
          {_contextItem.contextController.isUnsaved
            ? <i className="unsaved-feedback"/>
            : ''}
          <i className={'fa ' + iconClass + ' type-icon'}/>
          {_contextItem.contextTitle}
          <span className='close' onClick={function(_e) {
            _e.stopPropagation();
            self.closeContext(_contextItem);
          }}>
    ...
    renderContext (_contextSpec) {
      ...
      return <ContextClass ref={_contextSpec.contextID} width={this.state.width} height={this.state.height - this.getTabContextOffsetTopByDS()} contextId={_contextSpec.contextId} contextType={_contextSpec.contextType} runningState={running} contextController={_contextSpec.contextController}/>;
    },
    ...
    render: function() {
      ...
      let elementHighlighterStyle = {
        "border-width": this.props.boundaryBorderSize
      };
      ...
      return (
        <section className="DocumentStage tab-support black" style={rootStyle}>
          ...
          <div className='element-highlighter' ref='element-highlighter' style={elementHighlighterStyle}/>
          ...
          <div className='unboxing-zone' ref='unboxing-zone'></div>
        </section>
      )
    }
  });
