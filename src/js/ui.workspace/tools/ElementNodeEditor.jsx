...
var ElementNodeEditor = React.createClass({
  ...
  onClickReset () {
    this.refs['profile-set']
      .resetAll();
    this.refs['spec-set']
      .resetAll();
    this.refs['tag-attrs-set']
      .resetAll();
  },
      ...
      if (_eventData.refPath[1] === 'profile-set') {
        switch (_eventData.name) {
          case "Name":
            this.state
              .contextController
              .modifyElementProperty(elementNode.id, 'Name', changedData);
            break;
        }
      }
      ...
      if (_eventData.refPath[1] === 'elementDOMSpec') {
        switch (_eventData.name) {
          case "TagName":
            this.state
              .contextController
              .modifyElementAttribute(elementNode.id, 'tagName', changedData);
            break;
          case "Id":
            this.state
              .contextController
              .modifyElementAttribute(elementNode.id, 'id', changedData);
            break;
          case "Classes":
            this.state
              .contextController
              .modifyElementAttribute(elementNode.id, 'class', changedData);
            break;
          case "Text":
            this.state
              .contextController
              .modifyElementAttribute(elementNode.id, 'text', changedData);
            break;
          case "Comment":
            this.state
              .contextController
              .modifyElementProperty(elementNode.id, 'Comment', changedData);
            break;
        }
      ...
      if (_eventData.refPath[1] === 'tagAttribute') {
        switch (_eventData.name) {
          case "InlineStyle":
            this.state
              .contextController
              .modifyElementAttribute(elementNode.id, 'style', changedData);
            break;
          default:
            this.state
              .contextController
              .modifyElementAttribute(elementNode.id, _eventData.name, changedData);
        }
      }
      ...
      if (_eventData.refPath[1] === 'emptyTypeProps') {
        switch (_eventData.name) {
          case "RefferenceType":
            this.state
              .contextController
              .modifyElementProperty(elementNode.id, 'refferenceType', changedData);
            break;
          case "RefferenceTarget":
            this.state
              .contextController
              .modifyElementProperty(elementNode.id, 'refferenceTarget', changedData);
            break;
        }
      }
    }
  ...
  render () {
    var rootClasses = [
      'ElementNodeEditor', this.props.config.theme, this.getMySizeClass()
    ];
    ...
    return (
      <div className={rootClasses.join(' ')}>
        <div className='wrapper'>
          <div className='body'>
            {elementNode !== null
              ? this.renderEditParts(elementNode)
              : "No focused."}
          </div>
