...
var HTMLDOMSpec = React.createClass({
  ...
  onThrowCatcherRenameField (_eventData, _pass) {
    if (_eventData.refPath[1] === "dataAttribute") {
      ...
      if (/data-\w+/.test(_eventData.current)) {
        ...
        this.props
          .elementNode
          .renameAttribute(_eventData.past, _eventData.current);
      } else {
  ...
  onThrowCatcherNewFieldAdd (_eventData, _pass) {
    if (_eventData.refPath[0] === "dataAttribute") {
      ...
      this.props
        .elementNode
        .setAttribute("data-" + Math.floor(Math.random() * 99999), "");
  ...
  onThrowCatcherRemoveField (_eventData, _pass) {
    if (_eventData.refPath[1] === "dataAttribute") {
      ...
      this.props
        .elementNode
        .removeAttribute(_eventData.fieldName);
  ...
  render () {
    ...
    var rootClasses = [
      'part', this.props.theme, this.getMySizeClass()
    ];
    ...
    if (elementNode.getType() === 'string') {
      ...
      var targetTagSpecIndex = _.findIndex(htmlTag, {
      ...
      tagAttributeFieldSet.push({
        "name": "title",
        title: "tooltipMsg",
        "initialValue": elementNode.getAttribute('title') || '',
        type: "input",
        "enterable": true
      })
      ...
      elementAttrKeys.map(function(_key) {
        if (/^data-.*/.test(_key)) {
          dataAttributeFieldSet.push({
            "name": _key,
            title: _key,
            "initialValue": elementNode.getAttribute(_key) || '',
            type: "input",
            "enterable": true,
            "deletable": true,
            editorableFieldName: true
          });
        }
      });
    }
    ...
    return (
      <div className={rootClasses.join(' ')}>
        ...
        {elementNode.getType() !== 'string'
          ? <HorizonFieldSet title="Tag 지원 속성" theme={this.props.theme} nameWidth={130} fields={tagAttributeFieldSet} ref='tagAttribute'/>
          : ''}
        {elementNode.getType() !== 'string'
          ? <HorizonFieldSet title="Tag 명시 데이터" theme={this.props.theme} nameWidth={130} fields={dataAttributeFieldSet} extendable={true} ref='dataAttribute'/>
          : ''}
      </div>
    );
  }
