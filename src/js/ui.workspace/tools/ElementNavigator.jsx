...
var ENNode = React.createClass({
  ...
  componentWillMount: function() {
    ...
    this.state.childCount = childs !== null
      ? childs.length
      : 0;
    ...
    if (!this.state.collapse || this.childCount > 0) {
      childs
        .forEach(function(childNode) {
          childsNodes.push(
            <ENNode name={childNode.name} childs={childNode.childs}/>
          );
        });
    }
    return (

      <li className={this.state.collapse
        ? 'collapse'
        : ''}>
        {this.state.childCount > 0
          ? <i className="folding fa fa-play" onClick={this.toggleFolding}></i>
          : null}
