...
var DocumentCSSEditor = React.createClass({
  ...
  render () {
    var rootClasses = [
      'DocumentCSSEditor', this.props.theme, this.getMySizeClass()
    ];

    var targetDocument = this.state.document;

    if (targetDocument === null) 
      return <div>No focused a Document</div>;
    
    return (
      <div className={rootClasses.join(' ')}>
        <HorizonField fieldName='css' title='Document Stylesheet' theme="dark" enterable={true} type={'ace'} ref={'css-field'} onChange={this.onChange} defaultValue={targetDocument.getPageCSS()} height='100%' lang='css' editorId='document-css-editor' nameWidth={0}/>
      </div>
    );
  }
});
