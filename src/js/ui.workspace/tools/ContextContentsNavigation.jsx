...
var ContextContentsNavigation = React.createClass({
  ...
  renderElementNode (_elementNode, _depth) {
    ...
    if (_elementNode.children.length > 0) 
      hasChildren = true;
    
    return (
      <li>
        <div className={'element-node ' + selectedClass} onClick={function() {
          self.clickElementNode(_elementNode)
        }} onMouseEnter={function() {
          self.menterElementNode(_elementNode)
        }} onMouseLeave={function() {
          self.mleaveElementNode(_elementNode)
        }}>
          {this.renderElementVisibility(_elementNode, indentBlocks)}
          {isEmptyType
            ? this.renderEmptyInfo(_elementNode, indentBlocks)
            : ''}
        </div>

        {hasChildren
          ? this.renderElementNodeChildren(_elementNode.children, _depth + 1)
          : ''}
      </li>
    );
  },
  ...
  renderTreeWrapper () {
    ...
    return (
      <div className='tree-wrapper'>
        ...
        <ul>
          {elementNode instanceof ElementNode
            ? this.renderElementNode(elementNode, 0)
            : <div/>}
        </ul>
  ...
  render () {
    var rootClasses = [
      'ContextContentsNavigation', this.props.config.theme, this.getMySizeClass()
    ];
