  ...
  render() {
    ...
    var documentInfo = [
      {"name": "DocumentID", title: "문서 ID", "initialValue": targetDocument.getDocumentID(), enterable: false},
      {
        "name": "DocumentName",
        title: "문서 명",
        "initialValue": targetDocument.getDocumentName(),
        enterable: true,
        type: "input"
      },
      {
        "name": "DocumentTitle",
        title: "문서 제목",
        "initialValue": targetDocument.getDocumentTitle(),
        enterable: true,
        type: "input"
      },
      {
        "name": "DocumentType",
        title: "문서 타입",
        "initialValue": targetDocument.getType(),
        enterable: true,
        type: "select",
        options: [{value: 'layout'}, {value: 'contents'}]
      }
    ];
    ...
    var documentState = [
      {
        "name": "lastElementId",
        title: "LEID",
        "initialValue": targetDocument.getLastElementId(),
        enterable: false
      },
    ...
    return (
      <div className={rootClasses.join(' ')}>
        <HorizonFieldSet title="Document Profile" theme='dark' nameWidth={80} fields={ documentInfo }
                         ref='DocumentProfile'/>
        <HorizonFieldSet title="Document State" theme='dark' nameWidth={80} fields={ documentState }
                         ref='DocumentState'/>
      </div>
    );
  }
